...
void Spectrogram_Format(q15_t *buf)
{
	...
	//singal between 0 and 4096, never negative
...
void Spectrogram_Compute(q15_t *samples, q15_t *melvec)
{
	...
	//           --> Pointwise product
	//           Complexity: O(<n>) vexteur On matric On^2.4 au mieux
	//           Number of cycles: <TODO>
	//			 In place
	//start_cycle_count();
	arm_mult_q15(samples, hamming_window, buf, SAMPLES_PER_MELVEC);
	//stop_cycle_count("Step_1.0");
	...
	//           Complexity: O(<nlogn>) n² si naivf algo
	//           Number of cycles: <TODO>

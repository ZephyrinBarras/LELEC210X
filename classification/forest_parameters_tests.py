...
max_depth_begin = 10 # au départ on avait mis 20
max_depth_end = 100
max_depth_step = 10

n_estimators_arange = np.arange(n_estimators_begin, n_estimators_end, n_estimators_step)  # ICI LA RANGE DU PARAM NMEL
max_depth_arange = np.arange(max_depth_begin, max_depth_end,
                             max_depth_step)  # ICI LA RANGE DU PARAM MELVEC_LENGTH

# La création des matrices a été adaptée à la modification des boucles i et j
accuracy_matrix = np.zeros((len(max_depth_arange), len(n_estimators_arange)))
std_matrix = np.zeros((len(max_depth_arange), len(n_estimators_arange)))


for i in max_depth_arange:
    for j in n_estimators_arange:
        print("Nouvelle boucle pour : max_depth = {}, n_estimators = {}".format(i, j))

        a_spec = audio_signal_troncated(a[1], i_melvec_length=24, j_n_melvecs=32, Nft=512, fs_down=fs_down)
        b_spec = audio_signal_troncated(b[1], i_melvec_length=24, j_n_melvecs=32, Nft=512, fs_down=fs_down)
        c_spec = audio_signal_troncated(c[1], i_melvec_length=24, j_n_melvecs=32, Nft=512, fs_down=fs_down)
        d_spec = audio_signal_troncated(d[1], i_melvec_length=24, j_n_melvecs=32, Nft=512, fs_down=fs_down)
        e_spec = audio_signal_troncated(e[1], i_melvec_length=24, j_n_melvecs=32, Nft=512, fs_down=fs_down)
        ...
        for _ in range(n_splits):
            ...
            for test_elem in range(len(X_test)):
                ...
                X_val_spec.append(add_lol_effects_and_specgram(X_test[test_elem],elem_bg, i_melvec_length=24, j_n_melvecs=32, Nft=512, fs_down=fs_down))
            ...
            for train_elem in range(len(X_train)):
                ...
                x = x / np.linalg.norm(x)
                spec = np.ravel(np.log(np.abs(melspecgram(x, Nmel=32, mellength=24, fs_down=fs_down))))
        ...
        accuracy_matrix[(i - max_depth_begin) // max_depth_step][(j - n_estimators_begin) // n_estimators_step] = mean_acc

        print("max_depth : {}, n_estimators : {}, mean accuracy of the {}-splits : {}".format(i, j, n_splits,100 * mean_acc))
        print("=====================================")

plt.imshow(accuracy_matrix, cmap='hot', extent=[n_estimators_begin, n_estimators_end, max_depth_begin, max_depth_end])
plt.colorbar()
plt.title("Exactitude")
plt.xlabel("n_estimators")
plt.ylabel("max_depth")
plt.savefig("accuracy_matrix_forest_big_span.png")
plt.show()

...
for i in range(len(data1_list)):
    ...
    spec = melspecgram(x, 20, 20,512, fs_down, fs_down=fs_down)
    a = spec[:,0:10]
    b = spec[:,10:20]
    X_train_spec.append(np.ravel(np.column_stack((np.mean(a,axis=1),np.mean(b,axis=1)))))

spec =melspecgram(signal[0], 20, 20,512, fs_down, fs_down=fs_down)
a = spec[:,0:10]
b = spec[:,10:20]
plt.plot(np.mean(a,axis=1))
plt.show()
print(np.ravel(np.column_stack((np.mean(a,axis=1),np.mean(b,axis=1)))))

model = RandomForestClassifier(100)

for i in range(len(X_train_spec)):
    X_train_spec[i] = X_train_spec[i]/np.linalg.norm(X_train_spec[i])
X_train, X_test, y_train, y_test = train_test_split(X_train_spec, data2_list, test_size=0.3)
model.fit(X_train,y_train)

prevision = model.predict(X_test)
print(compute_accuracy(prevision, y_test))

pickle.dump(model, open("./model_mean.pickle", "wb"))
